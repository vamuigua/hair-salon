...
describe(Stylist) do
  ...
  describe("#clients") do
    it("returns an array of clients for that stylist") do
      test_stylist = Stylist.new({:stylist_name => "Jane", :id => nil})
      test_stylist.save()
      test_client = Client.new({:client_name => "Mercy", :id => nil})
      test_client.save()
      test_client2 = Client.new({:client_name => "Joy", :id => nil})
      test_client2.save()
      test_stylist.add_client(test_client.id())
      test_stylist.add_client(test_client2.id())
  ...
  describe("#delete") do
    it("deletes a Stylist's clients from the database") do
      test_stylist = Stylist.new({:stylist_name => "Jane", :id => nil})
      test_stylist.save()
      test_client1 = Client.new({:client_name => "Faith", :id => nil})
      test_client1.save()
      test_client2 = Client.new({:client_name => "Mary", :id => nil})
      test_client2.save()
      test_stylist.delete()
      expect(Stylist.all()).to(eq([]))
    end
